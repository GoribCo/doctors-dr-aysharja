...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="mb-4 text-xs font-semibold uppercase tracking-[0.22em] text-teal-700 dark:text-teal-300">{children}</p>
}
...
export default function HomeClient({ doctorContent: initialContent, config }: Props) {
  ...
  return <div className="homepage" data-visible={visible}>
    ...
    <main>
      ...
      <section className="intro-section content-section reveal" aria-labelledby="intro-title"><div><SectionLabel>{text(home, 'introLabel')}</SectionLabel><h2 id="intro-title">{text(home, 'introTitle')}</h2></div><div className="intro-text"><p>{text(home, 'introText')}</p><Link href="/profile" className="text-link">{text(home, 'learnMore')} <ArrowIcon /></Link></div></section>

      <section className="services-section content-section reveal" aria-labelledby="services-title"><div className="section-heading"><div><SectionLabel>{text(home, 'servicesLabel')}</SectionLabel><h2 id="services-title">{text(home, 'servicesTitle')}</h2></div><Link href="/services" className="text-link desktop-link">{text(home, 'viewServices')} <ArrowIcon /></Link></div><div className="service-grid">{services.map((service, index) => <Link href="/services" className="service-item" key={service}><span className="service-number">0{index + 1}</span><h3>{service}</h3><span className="service-arrow" aria-hidden="true">&#8599;</span></Link>)}</div><Link href="/services" className="text-link mobile-link">{text(home, 'viewServices')} <ArrowIcon /></Link></section>

      <section className="visit-section content-section reveal" aria-labelledby="visit-title"><div className="visit-card"><div><SectionLabel>{text(home, 'chamberLabel')}</SectionLabel><h2 id="visit-title">{text(home, 'chamberTitle')}</h2></div><div className="visit-details"><div><span className="detail-label">{text(home, 'chamberLabel')}</span><strong>{text(home, 'chamberName')}</strong></div><div><span className="detail-label">{text(home, 'addressLabel', 'Address')}</span><strong>{text(home, 'chamberAddress')}</strong></div><div><span className="detail-label">{text(home, 'availabilityLabel')}</span><strong>{text(home, 'consultationDays')}</strong></div><a href={phoneHref} className="button button-light"><PhoneIcon /> {text(home, 'callToBook')}</a></div></div></section>

      <section className="credentials-section content-section reveal" aria-labelledby="credentials-title"><SectionLabel>{text(home, 'credentialsLabel')}</SectionLabel><h2 id="credentials-title" className="sr-only">{text(home, 'credentialsLabel')}</h2><div className="credential-list">{credentials.map(credential => <span key={credential}>{credential}</span>)}</div></section>
